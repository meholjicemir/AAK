...
function updateSigninStatus(isSignedIn){if(isSignedIn){authorizeButton.style.display='none';signoutButton.style.display='block';var email=gapi.auth2.getAuthInstance().currentUser.Ab.w3.U3;var token=gapi.auth2.getAuthInstance().currentUser.Ab.Zi.id_token;ValidateUser(email,token,gapi.auth2.getAuthInstance().currentUser.Ab.Zi.access_token)}else{authorizeButton.style.display='block';signoutButton.style.display='none'}}
